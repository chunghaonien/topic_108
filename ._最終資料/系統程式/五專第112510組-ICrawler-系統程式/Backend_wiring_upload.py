...
async def upload_data(user_id, url, scraped_data):
    uri = "ws://140.131.114.149:80"  # 請替換為虛擬機的 IP 地址和端口

    async with websockets.connect(uri) as websocket:
        message = f"upload, {user_id}, {url}, {scraped_data}"
        await websocket.send(message)
        response = await websocket.recv()
        return response

async def main_async(user_id, url, scraped_data):
    response = await upload_data(user_id, url, scraped_data)
    print(f"{response}")
    # 在這裡進行其他的操作
...
async def main(): 
    ...
    parser.add_argument("url", type=str, help="Scraped_Url for Database")
    ...
    await asyncio.gather(main_async(args.user_id, args.url, args.scraped_data))
