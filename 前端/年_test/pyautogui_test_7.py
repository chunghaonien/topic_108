...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.keyboard_actions = []  # 存放鍵盤動作
        self.scroll_actions = []    # 存放滾輪動作
    ...
    def on_scroll(self, x, y, dx, dy):
        ...
        self.scroll_actions.append((x, y, dx, dy))

    def on_press(self, key):
        try:
            key_text = f"按下按鍵：{key.char}"
            self.keyboard_text_edit.append(key_text)
            self.keyboard_actions.append(key_text)
        ...
        if file_name:
            with open(file_name, 'w') as file:
                for item in self.clicked_positions:
                    file.write(f"滑鼠點擊：{item}\n")

                for action in self.keyboard_actions:
                    file.write(f"鍵盤動作：{action}\n")

                for action in self.scroll_actions:
                    file.write(f"滾輪動作：{action}\n")
