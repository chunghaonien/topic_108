...
from PyQt5.QtCore import Qt, pyqtSignal, pyqtSlot  # 引入 pyqtSignal
...
class WebBrowserWindow(QMainWindow):
    text_selected = pyqtSignal(str)  # 創建一個自定義信號
    ...
    def handle_selection_changed(self):
        ...
        if selected_text:
            ...
            self.text_selected.emit(selected_text)  # 發出自定義信號
    ...
    # 定義 handle_js_call 方法，用於處理 JavaScript 呼叫並將反白內容保存到事件追蹤器
    @pyqtSlot(str)  # 使用 pyqtSlot 修飾槽函數
...
if __name__ == '__main__':
    ...
    web_browser_window.text_selected.connect(main_window.handle_js_call)  # 連接信號到槽
    app.exec_()
